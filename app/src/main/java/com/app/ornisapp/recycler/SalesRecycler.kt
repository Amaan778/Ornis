...
class SalesRecycler : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_sales_recycler)
    ...
    private fun loadSalesForMonth(month: String) {
        val ref = FirebaseDatabase.getInstance().getReference("sales").child(month)
        salesList.clear()
        adapter.notifyDataSetChanged()
        noDataText.visibility = View.GONE
        ...
        ref.addListenerForSingleValueEvent(object : ValueEventListener {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@SalesRecycler, "Error: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
}